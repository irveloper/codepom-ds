const d={title:"Foundations/Accessibility"},e='<span style="display:inline-flex;align-items:center;gap:5px;color:#4ADE80;font-size:12px;font-weight:600;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"><path d="M20 6L9 17l-5-5"/></svg>PASS</span>',r='<span style="display:inline-flex;align-items:center;gap:5px;color:#F87171;font-size:12px;font-weight:600;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg>FAIL</span>',p='<span style="display:inline-flex;align-items:center;gap:5px;color:#FB923C;font-size:12px;font-weight:600;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>WARN</span>',l=o=>`<span style="display:inline-block;width:20px;height:20px;background:${o};border-radius:4px;border:1px solid rgba(255,255,255,0.1);vertical-align:middle;margin-right:6px;"></span>`,t={name:"Contrast Audit",parameters:{layout:"fullscreen"},render:()=>`
    <div style="font-family:'Space Grotesk',sans-serif;background:#0B132B;padding:40px;min-height:100vh;color:#F8FAFC;">
      <div style="max-width:820px;margin:0 auto;">
        ...
        <div style="border:1px solid #1C2541;border-radius:12px;overflow:hidden;margin-bottom:40px;">
          ...
          ${[{fg:"#F8FAFC",bg:"#0B132B",label:"text-primary on bg",ratio:"17.6",aN:e,aL:e,use:"Body copy, headings"},{fg:"#94A3B8",bg:"#0B132B",label:"text-secondary on bg",ratio:"7.2",aN:e,aL:e,use:"Labels, captions"},{fg:"#64748B",bg:"#0B132B",label:"text-tertiary on bg",ratio:"3.9",aN:r,aL:e,use:"Large text / decorative only"},{fg:"#3A506B",bg:"#0B132B",label:"navy-700 on bg",ratio:"2.2",aN:r,aL:r,use:"Borders & dividers — NO text"},{fg:"#FBB040",bg:"#0B132B",label:"amber-400 on bg",ratio:"9.97",aN:e,aL:e,use:"Primary interactive"},{fg:"#00DFFF",bg:"#0B132B",label:"cyan-400 on bg",ratio:"11.4",aN:e,aL:e,use:"Accent / status"},{fg:"#4ADE80",bg:"#0B132B",label:"success on bg",ratio:"10.6",aN:e,aL:e,use:"Success states"},{fg:"#FB923C",bg:"#0B132B",label:"warning on bg",ratio:"8.1",aN:e,aL:e,use:"Warning states"},{fg:"#F87171",bg:"#0B132B",label:"error on bg",ratio:"6.7",aN:e,aL:e,use:"Error states"},{fg:"#0B132B",bg:"#FBB040",label:"navy on amber (buttons)",ratio:"9.97",aN:e,aL:e,use:"Primary button text"},{fg:"#0B132B",bg:"#00DFFF",label:"navy on cyan (buttons)",ratio:"11.4",aN:e,aL:e,use:"Accent button text"}].map(o=>`
            <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr;padding:12px 16px;border-bottom:1px solid #0e1627;align-items:center;">
              <span style="font-size:13px;color:#F8FAFC;">${l(o.fg)}${o.label}</span>
              <span style="font-family:'Space Mono',monospace;font-size:13px;color:#FBB040;">${o.ratio}:1</span>
              <span>${o.aN}</span>
              <span>${o.aL}</span>
              <span style="font-size:12px;color:#64748B;">${o.use}</span>
            </div>
          `).join("")}
        </div>
  ...
  `},a={name:"Focus Rings",parameters:{layout:"centered"},render:()=>`
  ...
  `},s={name:"Reduced Motion",parameters:{layout:"centered"},render:()=>`
    <style>
...
<span style="color:#00DFFF;">@media</span> <span style="color:#FBB040;">(prefers-reduced-motion: reduce)</span> {
  ...
  `},i={name:"ARIA Patterns",parameters:{layout:"fullscreen"},render:()=>`
    <div style="font-family:'Space Grotesk',sans-serif;background:#0B132B;padding:40px;min-height:100vh;color:#F8FAFC;">
      <div style="max-width:780px;margin:0 auto;">
        ...
        <div style="display:flex;flex-direction:column;gap:12px;">
          ${[{component:"Button",attrs:['role="button" (implicit on <button>)',"aria-disabled when inactive","aria-pressed for toggles","aria-expanded for triggers"],status:"pass"},{component:"Alert",attrs:['role="alert"','aria-live="polite" for non-urgent','aria-live="assertive" for critical'],status:"pass"},{component:"Modal",attrs:['role="dialog"','aria-modal="true"',"aria-labelledby → title id","Focus trapped inside","Escape closes + returns focus"],status:"pass"},{component:"Toast",attrs:['role="status" / aria-live="polite"',"Auto-dismiss ≥ 5s or user-controllable"],status:"pass"},{component:"Tabs",attrs:['role="tablist" on wrapper','role="tab" on each tab','role="tabpanel" on content',"aria-selected on active tab","aria-controls ↔ aria-labelledby"],status:"pass"},{component:"Breadcrumb",attrs:['<nav aria-label="Breadcrumb">',"<ol> with <li> items",'aria-current="page" on last item'],status:"pass"},{component:"Pagination",attrs:['<nav aria-label="Pagination">','aria-current="page" on active',"aria-label on prev/next buttons","aria-disabled on disabled"],status:"pass"},{component:"Tooltip",attrs:['role="tooltip" on tip',"aria-describedby on trigger","Hidden with aria-hidden when closed","Shown on focus AND hover"],status:"pass"},{component:"Popover",attrs:["aria-haspopup on trigger","aria-expanded on trigger","aria-controls → panel id","Escape closes + returns focus"],status:"pass"},{component:"Avatar",attrs:["alt text on <img>","aria-label on status dot","sr-only name when initials-only"],status:"pass"},{component:"Table",attrs:["<table> with <thead>/<tbody>",'<th scope="col"> on headers',"aria-label on icon buttons","aria-sort for sortable columns (TODO)"],status:"warn"},{component:"Form inputs",attrs:["<label> associated via for/id or wrapping","aria-required for required fields","aria-invalid + aria-describedby on error (TODO)"],status:"warn"}].map(o=>`
            <div style="background:#08192E;border:1px solid #1C2541;border-radius:12px;padding:16px 18px;display:grid;grid-template-columns:160px 1fr 80px;gap:16px;align-items:start;">
              <div>
                <span style="font-size:14px;font-weight:600;color:#F8FAFC;">${o.component}</span>
              </div>
              <ul style="margin:0;padding-left:16px;display:flex;flex-direction:column;gap:4px;">
                ${o.attrs.map(n=>`<li style="font-size:13px;color:#94A3B8;font-family:'Space Mono',monospace;font-size:11px;line-height:1.6;">${n}</li>`).join("")}
              </ul>
  ...
  render: () => \`
    ...
    fg: '#64748B',
    bg: '#0B132B',
    label: 'text-tertiary on bg',
    ratio: '3.9',
    aN: fail,
    aL: pass,
    use: 'Large text / decorative only'
  }, {
    fg: '#3A506B',
    bg: '#0B132B',
    label: 'navy-700 on bg',
    ratio: '2.2',
    aN: fail,
    aL: fail,
    use: 'Borders & dividers — NO text'
  }, {
    fg: '#FBB040',
    bg: '#0B132B',
    label: 'amber-400 on bg',
    ratio: '9.97',
    aN: pass,
    aL: pass,
    use: 'Primary interactive'
  }, {
    fg: '#00DFFF',
    bg: '#0B132B',
    label: 'cyan-400 on bg',
    ratio: '11.4',
    aN: pass,
    aL: pass,
    use: 'Accent / status'
...
<span style="color:#00DFFF;">@media</span> <span style="color:#FBB040;">(prefers-reduced-motion: reduce)</span> {
    ...
    <div style="font-family:'Space Grotesk',sans-serif;background:#0B132B;padding:40px;min-height:100vh;color:#F8FAFC;">
      <div style="max-width:780px;margin:0 auto;">
        ...
        <div style="display:flex;flex-direction:column;gap:12px;">
          \${[{
    component: 'Button',
    attrs: ['role="button" (implicit on <button>)', 'aria-disabled when inactive', 'aria-pressed for toggles', 'aria-expanded for triggers'],
    status: 'pass'
  }, {
    component: 'Alert',
    attrs: ['role="alert"', 'aria-live="polite" for non-urgent', 'aria-live="assertive" for critical'],
    status: 'pass'
  }, {
    component: 'Modal',
    attrs: ['role="dialog"', 'aria-modal="true"', 'aria-labelledby → title id', 'Focus trapped inside', 'Escape closes + returns focus'],
    status: 'pass'
  }, {
    component: 'Toast',
    attrs: ['role="status" / aria-live="polite"', 'Auto-dismiss ≥ 5s or user-controllable'],
    status: 'pass'
  }, {
    component: 'Tabs',
    attrs: ['role="tablist" on wrapper', 'role="tab" on each tab', 'role="tabpanel" on content', 'aria-selected on active tab', 'aria-controls ↔ aria-labelledby'],
    status: 'pass'
  }, {
    component: 'Breadcrumb',
    attrs: ['<nav aria-label="Breadcrumb">', '<ol> with <li> items', 'aria-current="page" on last item'],
    status: 'pass'
  }, {
    component: 'Pagination',
    attrs: ['<nav aria-label="Pagination">', 'aria-current="page" on active', 'aria-label on prev/next buttons', 'aria-disabled on disabled'],
    status: 'pass'
  }, {
    component: 'Tooltip',
    attrs: ['role="tooltip" on tip', 'aria-describedby on trigger', 'Hidden with aria-hidden when closed', 'Shown on focus AND hover'],
    status: 'pass'
  }, {
    component: 'Popover',
    attrs: ['aria-haspopup on trigger', 'aria-expanded on trigger', 'aria-controls → panel id', 'Escape closes + returns focus'],
    status: 'pass'
  }, {
    component: 'Avatar',
    attrs: ['alt text on <img>', 'aria-label on status dot', 'sr-only name when initials-only'],
    status: 'pass'
  }, {
    component: 'Table',
    attrs: ['<table> with <thead>/<tbody>', '<th scope="col"> on headers', 'aria-label on icon buttons', 'aria-sort for sortable columns (TODO)'],
    status: 'warn'
  }, {
    component: 'Form inputs',
    attrs: ['<label> associated via for/id or wrapping', 'aria-required for required fields', 'aria-invalid + aria-describedby on error (TODO)'],
    status: 'warn'
  }].map(r => \`
            <div style="background:#08192E;border:1px solid #1C2541;border-radius:12px;padding:16px 18px;display:grid;grid-template-columns:160px 1fr 80px;gap:16px;align-items:start;">
              <div>
  ...
  \`
}`,...i.parameters?.docs?.source}}};const c=["ContrastAudit","FocusRings","ReducedMotion","ARIAPatterns"];export{i as ARIAPatterns,t as ContrastAudit,a as FocusRings,s as ReducedMotion,c as __namedExportsOrder,d as default};
