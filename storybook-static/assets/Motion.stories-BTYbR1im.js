const t={title:"Foundations/Motion"},o={name:"Scale",parameters:{layout:"fullscreen"},render:()=>`
    <style>
    ...
    <div style="font-family:'Space Grotesk',sans-serif;background:#0B132B;color:#F8FAFC;padding:40px;min-height:100vh;">
      <div style="max-width:780px;margin:0 auto;">
        ...
        <div style="border:1px solid #1C2541;border-radius:12px;overflow:hidden;margin-bottom:40px;">
          ${[{token:"--cp-duration-instant",val:"0ms",use:"State toggles (checked, active)"},{token:"--cp-duration-fast",val:"100ms",use:"Hover, focus ring, icon swap"},{token:"--cp-duration-normal",val:"200ms",use:"Fade in/out, color change"},{token:"--cp-duration-slow",val:"350ms",use:"Slide, expand, height change"},{token:"--cp-duration-enter",val:"250ms",use:"Elements entering the view"},{token:"--cp-duration-exit",val:"150ms",use:"Elements leaving (always faster)"}].map(e=>`
            <div style="display:flex;align-items:center;gap:16px;padding:13px 18px;border-bottom:1px solid #0e1627;">
              <span style="font-family:'Space Mono',monospace;font-size:13px;color:#FBB040;min-width:220px;">${e.token}</span>
              <span style="font-family:'Space Mono',monospace;font-size:12px;color:#64748B;min-width:52px;">${e.val}</span>
              <span style="font-size:13px;color:#94A3B8;">${e.use}</span>
            </div>
          `).join("")}
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:40px;">
          ${[{token:"--cp-ease-out",val:"cubic-bezier(0.0,0.0,0.2,1)",use:"Enter / decelerate",hint:"Things entering screen. Fast start, soft land."},{token:"--cp-ease-in",val:"cubic-bezier(0.4,0.0,1.0,1)",use:"Exit / accelerate",hint:"Things leaving screen. Slow start, fast exit."},{token:"--cp-ease-in-out",val:"cubic-bezier(0.4,0.0,0.2,1)",use:"Move / standard",hint:"Moving between states. Balanced feel."},{token:"--cp-ease-spring",val:"cubic-bezier(0.34,1.56,0.64,1)",use:"Spring / overshoot",hint:"Playful bounce. Use sparingly for delight."}].map(e=>`
            <div style="background:#08192E;border:1px solid #1C2541;border-radius:12px;padding:20px;">
              <div style="font-family:'Space Mono',monospace;font-size:12px;color:#FBB040;margin-bottom:8px;">${e.token}</div>
              <div style="font-weight:600;font-size:14px;margin-bottom:4px;">${e.use}</div>
              <div style="font-size:13px;color:#64748B;margin-bottom:14px;line-height:1.5;">${e.hint}</div>
              <div style="font-family:'Space Mono',monospace;font-size:10px;color:#3A506B;word-break:break-all;">${e.val}</div>
            </div>
          `).join("")}
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
          ${[{label:"fast (100ms)",dur:"100ms"},{label:"normal (200ms)",dur:"200ms"},{label:"slow (350ms)",dur:"350ms"},{label:"enter (250ms)",dur:"250ms"}].map(e=>`
            <div style="display:flex;align-items:center;gap:16px;">
              <span style="font-family:'Space Mono',monospace;font-size:12px;color:#64748B;min-width:120px;">${e.label}</span>
              <div class="cp-demo-track" style="flex:1;height:20px;background:#08192E;border:1px solid #1C2541;border-radius:4px;overflow:hidden;cursor:pointer;">
                <div class="cp-demo-bar" style="transition-duration:${e.dur};"></div>
              </div>
            </div>
          `).join("")}
        </div>
  ...
  render: () => \`
    ...
    <div style="font-family:'Space Grotesk',sans-serif;background:#0B132B;color:#F8FAFC;padding:40px;min-height:100vh;">
      <div style="max-width:780px;margin:0 auto;">
        ...
        <div style="border:1px solid #1C2541;border-radius:12px;overflow:hidden;margin-bottom:40px;">
          \${[{
    token: '--cp-duration-instant',
    val: '0ms',
    use: 'State toggles (checked, active)'
  }, {
    token: '--cp-duration-fast',
    val: '100ms',
    use: 'Hover, focus ring, icon swap'
  }, {
    token: '--cp-duration-normal',
    val: '200ms',
    use: 'Fade in/out, color change'
  }, {
    token: '--cp-duration-slow',
    val: '350ms',
    use: 'Slide, expand, height change'
  }, {
    token: '--cp-duration-enter',
    val: '250ms',
    use: 'Elements entering the view'
  }, {
    token: '--cp-duration-exit',
    val: '150ms',
    use: 'Elements leaving (always faster)'
  }].map(r => \`
            <div style="display:flex;align-items:center;gap:16px;padding:13px 18px;border-bottom:1px solid #0e1627;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:40px;">
          \${[{
    token: '--cp-ease-out',
    val: 'cubic-bezier(0.0,0.0,0.2,1)',
    use: 'Enter / decelerate',
    hint: 'Things entering screen. Fast start, soft land.'
  }, {
    token: '--cp-ease-in',
    val: 'cubic-bezier(0.4,0.0,1.0,1)',
    use: 'Exit / accelerate',
    hint: 'Things leaving screen. Slow start, fast exit.'
  }, {
    token: '--cp-ease-in-out',
    val: 'cubic-bezier(0.4,0.0,0.2,1)',
    use: 'Move / standard',
    hint: 'Moving between states. Balanced feel.'
  }, {
    token: '--cp-ease-spring',
    val: 'cubic-bezier(0.34,1.56,0.64,1)',
    use: 'Spring / overshoot',
    hint: 'Playful bounce. Use sparingly for delight.'
  }].map(r => \`
            <div style="background:#08192E;border:1px solid #1C2541;border-radius:12px;padding:20px;">
  ...
  \`
}`,...o.parameters?.docs?.source}}};const a=["Scale"];export{o as Scale,a as __namedExportsOrder,t as default};
